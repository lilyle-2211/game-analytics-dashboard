...
class DashboardExplainer:
    ...
    def _setup_openai(self):
        ...
        try:
            ...
            if not enable_ai:
                print("AI calls disabled - use command line --enable-ai or dashboard toggle to enable")
                return False
        ...
        try:
            ...
            if dataframe is not None and not dataframe.empty:
                ...
                if len(dataframe) > 200:
                    ...
                    data_sample = f"First 50 rows:\n{dataframe.head(50).to_string()}\n\n"
                    data_sample += f"Last 20 rows:\n{dataframe.tail(20).to_string()}\n\n"
                    data_sample += f"Summary statistics:\n{dataframe.describe().to_string()}\n\n"
                    ...
                    data_representation = data_info + f"Data sample (first 100 rows):\n{dataframe.head(100).to_string()}\n\nSummary statistics:\n{dataframe.describe().to_string()}"
                else:
                    # For smaller datasets, show all data
                    data_representation = data_info + f"Full data:\n{dataframe.to_string()}"
            else:
    ...
    def analyze_distribution_subplot(self, dataframe, column_name, description):
        ...
        try:
            ...
            distribution_data = dataframe.groupby(column_name)['num_player'].agg(['sum', 'count']).reset_index()
            distribution_data = distribution_data.sort_values('sum', ascending=False)
            distribution_data['percentage'] = (distribution_data['sum'] / distribution_data['sum'].sum() * 100).round(2)
            
            # Format the distribution data
            data_representation = f"Distribution of {column_name}:\n"
            data_representation += f"Total players: {distribution_data['sum'].sum():,}\n\n"
