...
def create_revenue_segmentation(df):
    ...
    segment_labels = [
        'Non-Payer', 'Low Spender', 'Medium Spender', 'High Spender',
        'Premium', 'VIP', 'Whale'
    ]
    ...
    for segment in segment_labels:
        ...
        row['pct_ltv_20_day_per_segment'] = (row['ltv_20_day'] / total_revenue) * 100 if total_revenue > 0 else 0
...
def create_retention_curve_data(df):
    ...
    try:
        ...
        params, _ = curve_fit(weibull, days_1_20, retention_fraction, 
...
def project_90_day_ltv_simple(segmentation_df, retention_df=None, raw_revenue_df=None):
    ...
    for _, row in segmentation_df.iterrows():
        ...
        avg_ltv_20_day_per_user = row.get('avg_ltv_20_day_per_user', ltv_20_day_total / num_users if num_users > 0 else 0)
        ...
        if retention_df is not None and not retention_df.empty and avg_daily_revenue > 0:
            try:
                ...
                if len(retention_rates) >= 15:  # Need sufficient data points
                    ...
                    if retention_vals[-1] > 0:
                        try:
                            ...
                            params, _ = curve_fit(weibull, days, retention_vals, 
                                                 p0=p0, bounds=bounds, sigma=1/weights)
                            lambda_param, k = params
                            
                            # Project to day 90 using the calculated daily revenue
                            projected_90 = avg_ltv_20_day_per_user  # Start with day 20 LTV
