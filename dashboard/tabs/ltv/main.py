...
def render_ltv_tab():
            ...
            if revenue_df is not None and not revenue_df.empty:
                ...
                if not segmentation_table.empty:
                    ...
                    with col_charts:
                        ...
                        segmentation_display_filtered = segmentation_display[segmentation_display['segment'] != 'Non-Payer'].copy()
                        
                        # Calculate percentages
                        total_users = segmentation_display['num_user'].sum()
                        total_revenue = segmentation_display['ltv_20_day'].sum()
                        
                        segmentation_display['user_pct'] = (segmentation_display['num_user'] / total_users) * 100
                        segmentation_display['revenue_pct'] = (segmentation_display['ltv_20_day'] / total_revenue) * 100
                        segmentation_display_filtered['user_pct'] = (segmentation_display_filtered['num_user'] / total_users) * 100
                        segmentation_display_filtered['revenue_pct'] = (segmentation_display_filtered['ltv_20_day'] / total_revenue) * 100
                        
                        # Sort by segment order
                        segmentation_display_filtered['segment_order'] = segmentation_display_filtered['segment_display'].map(
                        ...
                        with chart_col1:
                            ...
                            fig1 = px.bar(
                                ...
                                color='importance',
                                color_continuous_scale=[[0, '#C6DBEF'], [0.2, '#9ECAE1'], [0.4, '#6BAED6'], [0.6, '#4292C6'], [0.8, '#2171B5'], [1, '#08519C']]
                            )
    ...
    with st.container():
        st.markdown("<h3 style='color: #0066cc;'>Retention Rate Analysis</h3>", unsafe_allow_html=True)
        ...
        try:
            if retention_df is not None and not retention_df.empty:
                ...
                if not retention_curve_data.empty:
                    ...
                    with st.expander("Retention Rate Data - Actual vs Projected", expanded=False):
                        # Create table view of the data
                        table_data = retention_curve_data.pivot(index='day', columns='type', values='retention_rate').reset_index()
                        
                        # Clean up column names and fill NaN values
                        if 'Actual' in table_data.columns and 'Projected' in table_data.columns:
                            ...
                            table_data['Projected Retention (%)'] = table_data['Projected Retention (%)'].round(2)
                    ...
                    **How 90-day projection is calculated:**
                    ...
                    projection_table = project_90_day_ltv_simple(segmentation_table, retention_df, revenue_df)
                    
                    # Display simplified table with only requested columns
                    display_projection_cols = [
                        'segment', 'num_user', 'ltv_20_day', 'avg_daily_revenue_per_user',
                        'avg_ltv_20_day_per_user', 'projected_avg_90_day_per_user'
                    ]
                    
                    # Format the projection table for better display
                    projection_display = projection_table[display_projection_cols].copy()
                    projection_display.columns = [
                        'Segment', 'Number of Users', 'Total LTV (20 Days)', 'Avg Daily Revenue per User',
                        'Avg LTV per User (20 Days)', 'Projected Avg LTV per User (90 Days)'
                    ]
    ...
    if revenue_df is not None and not revenue_df.empty:
        segmentation_table = create_revenue_segmentation(revenue_df)
        if not segmentation_table.empty:
            with st.expander("LTV 1-20 Day by Segment - Detailed Table", expanded=False):
                st.markdown("**Complete LTV breakdown by segment showing daily progression from day 1 to day 20**")
    ...
    with st.expander("SQL Code Example & Sample Data", expanded=False):
        ...
        with col1:
            ...
            st.markdown("**Data Source:** `tactile-471816.data_analyst_test_local.users_view`, `tactile-471816.data_analyst_test_local.revenue_view`")
