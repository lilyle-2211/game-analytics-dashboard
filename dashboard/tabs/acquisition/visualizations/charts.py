...
def plot_timeseries(df_clean, explainer):
    ...
    # AI Analysis
    explanation = explainer.explain_chart("timeseries", timeseries_data, "Player acquisition by launch phase showing regional launch strategy")
...
def plot_weekly_patterns(df_clean, explainer):
    ...
    try:
        ...
        explanation = explainer.explain_chart("weekly_pattern", weekly_summary, "US player acquisition weekly patterns showing trends by day of week across different weeks")
        render_ai_insights(explanation, explainer)
...
def plot_demographics(df_clean, explainer):
    ...
    platform_explanation = explainer.analyze_distribution_subplot(df_us_dist, 'platform', "Mobile platform distribution for US players")
    gender_explanation = explainer.analyze_distribution_subplot(df_us_dist, 'gender', "Gender distribution for US players") 
    channel_explanation = explainer.analyze_distribution_subplot(df_us_dist, 'channel_type', "Acquisition channel distribution for US players")
    age_explanation = explainer.analyze_distribution_subplot(df_us_dist, 'age_group', "Age group distribution for US players")
    
    if any([platform_explanation, gender_explanation, channel_explanation, age_explanation]):
