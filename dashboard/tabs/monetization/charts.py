...
def render_iap_metrics(df, anomaly_df):
    ...
    if not anomaly_df.empty and "revenue_type" in anomaly_df.columns:
        anomaly_iap = anomaly_df[anomaly_df["revenue_type"] == "iap"][
            "transaction_value"
        ].sum()
...
def plot_iap_chart(df, anomaly_df, explainer):
    ...
    if not anomaly_df.empty and "revenue_type" in anomaly_df.columns:
        for _, anomaly_row in anomaly_df[
            anomaly_df["revenue_type"] == "iap"
        ].iterrows():
            ...
            if anomaly_date in chart_df["revenue_date"].values:
                ...
                chart_df = pd.concat(
                    [chart_df, new_row.to_frame().T], ignore_index=True
                )
    ...
    fig = px.line(
        ...
        y="iap_revenue",
        markers=True,
        labels={"revenue_date": "Date", "iap_revenue": "IAP Revenue ($)"},
    ...
    if not anomaly_df.empty and "revenue_type" in anomaly_df.columns:
        ...
        for _, anomaly_row in anomaly_df[
            ...
            anomaly_date = pd.to_datetime(anomaly_row["eventDate"])
            if anomaly_date in chart_df["revenue_date"].values:
                anomaly_value = chart_df[chart_df["revenue_date"] == anomaly_date][
                    "iap_revenue"
                ].iloc[0]
    ...
    explanation = explainer.explain_chart(
        "iap_revenue",
        df,
        "In-app purchase revenue analysis showing daily IAP performance and trends",
    )
...
def plot_ad_chart(df, explainer):
    ...
    fig = px.line(
        ...
        y="ad_revenue",
        markers=True,
        labels={"revenue_date": "Date", "ad_revenue": "Ad Revenue ($)"},
    )
    ...
    explanation = explainer.explain_chart(
        "ad_revenue",
        df,
        "Advertisement revenue analysis showing daily ad performance and user engagement patterns",
    )
    render_ai_insights(explanation, explainer)
...
def plot_revenue_per_user_metrics(df, anomaly_df):
    ...
    unique_iap_ad_users = df["total_unique_paying_users_all"].iloc[
        0
    ]  # All revenue users (IAP + AD)

    # Calculate IAP user conversion rate (IAP users / total active users)
    iap_conversion_rate = (
        unique_iap_users / unique_active_users if unique_active_users > 0 else 0
    )
    ...
    table_columns = ["revenue_date", "DAU", "total_revenue", "total_arpdau"]

    # Create display dataframe with only requested columns
    display_df = table_data[table_columns].rename(
        columns={
            "revenue_date": "Date",
            "DAU": "DAU",
    ...
    display_df["Revenue (IAP + AD)"] = display_df["Revenue (IAP + AD)"].map(
        "${:,.2f}".format
    )
...
def render_revenue_per_user_metrics_boxes(df, anomaly_df):
    ...
    if not anomaly_df.empty and "revenue_type" in anomaly_df.columns:
        ...
        _ = len(anomaly_df[anomaly_df["revenue_type"] == "iap"])
