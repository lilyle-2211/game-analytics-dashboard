...
from .queries import ANOMALY_TRANSACTIONS_QUERY, REVENUE_BY_SOURCE_QUERY
    ...
    with st.container():
        ...
        with col_insights:
            ...
            render_manual_insights(
                IAP_REVENUE_INSIGHTS, height=350, key_suffix="iap_revenue"
            )
...
def _render_ad_revenue_analysis(df, explainer):
    ...
    with st.container():
        ...
        with col_insights:
            st.markdown("**Key Notes**")
            render_manual_insights(
                AD_REVENUE_INSIGHTS, height=350, key_suffix="ad_revenue"
            )
...
def _render_revenue_per_user_analysis(df, anomaly_df):
    ...
    with st.container():
        ...
        with col_manual:
            st.markdown("**Key Notes**")
            render_manual_insights(
                REVENUE_PER_USER_INSIGHTS, height=300, key_suffix="revenue_per_user"
            )
