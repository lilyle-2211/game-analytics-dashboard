...
def plot_daily_active_users(explainer):
    ...
    if not df.empty:
        ...
        # Create DAU plot
        fig = px.line(df, x="date", y="daily_active_users", title="", markers=True)

        fig.update_layout(
            xaxis_title="Date", yaxis_title="Daily Active Users", height=300
        )
        ...
        try:
            ...
            total_unique_users = (
                return_df["total_unique_users"].iloc[0]
                if not return_df.empty and "total_unique_users" in return_df.columns
                else 0
            )
        except:
        ...
        st.info(
            f"**Average DAU: {avg_dau:.0f}** | **Peak DAU: {max_dau:,}** | **Total unique users: {total_unique_users:,}**"
        )
        ...
        explanation = explainer.explain_chart(
            "daily_active_users",
            dau_only_df,
            "Daily active users trend analysis focusing only on user count patterns",
        )
...
def plot_daily_engagement(explainer):
    ...
    if not df.empty:
        # Data preprocessing
        df["date"] = pd.to_datetime(df["date"])
        df["avg_levels_played_per_user"] = (
            df["total_levels_played"] / df["daily_active_users"]
        )
        df["avg_levels_completed_per_user"] = (
            df["total_levels_completed"] / df["daily_active_users"]
        )
        df["completion_rate"] = (
            df["total_levels_completed"] / df["total_levels_played"]
        ) * 100
        ...
        with plot_col2:
            fig2 = px.line(
                df,
                x="date",
                y="avg_levels_completed_per_user",
        ...
        explanation = explainer.explain_chart(
            "engagement_metrics",
            df,
            "Daily player engagement showing levels played, completed, and completion rates",
        )
        render_ai_insights(explanation, explainer)
    else:
...
def plot_daily_return_rate(explainer):
    ...
    if not df.empty:
        ...
        fig = px.line(df, x="date", y="daily_return_rate_pct", title="", markers=True)
        ...
        st.info(
            f"**Average return rate: {avg_return_rate:.1f}%** | **Peak return rate: {max_return_rate:.1f}%**"
        )
        ...
        explanation = explainer.explain_chart(
            "return_rate",
            return_rate_only_df,
            "Daily return rate percentage trends showing player retention day-to-day",
        )
        render_ai_insights(explanation, explainer)
    else:
...
def plot_two_week_retention(explainer):
    ...
    if not df.empty:
        ...
        display_df = df.copy()
        display_df["total_installed_users"] = display_df[
            "total_installed_users"
        ].astype(int)
        display_df["users_active_week_2"] = display_df["users_active_week_2"].astype(
            int
        )
        display_df["two_week_retention_pct"] = display_df[
            "two_week_retention_pct"
        ].round(2)
        ...
        explanation = explainer.explain_chart(
            "retention_analysis",
            df,
            "Two-week retention rates comparing soft launch vs official launch phases",
        )
        render_ai_insights(explanation, explainer)
    else:
...
def plot_progression_milestones(explainer):
    ...
    if not df.empty:
        ...
        display_df = df.copy()
        display_df["cumulative_avg_days_to_complete"] = (
            display_df["cumulative_avg_days_to_complete"].round(1).astype(str) + " days"
        )
        ...
        explanation = explainer.explain_chart(
            ...
            df,
            "Player progression milestone analysis showing completion times",
        )
        render_ai_insights(explanation, explainer)
    else:
